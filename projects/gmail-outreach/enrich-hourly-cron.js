...
async function main() {
  console.log('🫡 PE Enrichment Hourly Cron - Starting...\n');
  
  // Load targets
  const targets = JSON.parse(fs.readFileSync('_enrichment_targets.json', 'utf-8'));
  
  // Filter out already processed statuses
  const unprocessed = targets.filter(t => 
    t.status !== 'Contacted' && 
    t.status !== 'Enriched' && 
    t.status !== 'Researched' &&
  ...
  for (let idx = 0; idx < Math.min(15, unprocessed.length); idx++) {
    const target = unprocessed[idx];
  ...
  console.log('📊 HOURLY ENRICHMENT SUMMARY');
  console.log('='.repeat(60));
  ...
  console.log(`Next run will process ${Math.min(15, unprocessed.length - 15)} more firms.`);
