/**
 * Add Final New PE Firms - March 16, 2026 9:45 PM
 * Adding newly-found firms not in sheet
 */
...
const newFirms = [
  {
    ...
    domain: 'gladebrook.com',
    website: 'https://www.gladebrook.com',
    sector: 'Multi-sector',
    notes: 'South Florida-based. $4B+ AUM. Just raised $1B Gondola Fund (March 2026).',
  },
  {
    name: 'Corbel Capital Partners',
    domain: 'corbelcap.com',
    website: 'https://www.corbelcap.com',
    sector: 'Business Services, Lower Middle Market',
    notes: 'Los Angeles-based. $1B AUM. Structured debt/equity, lower middle market focus.',
  },
  {
    name: 'Aquarian Capital',
    domain: 'aquarianlp.com',
    website: 'https://aquarianlp.com',
    sector: 'Financial Services, Insurance',
    notes: 'Focus on retirement income & insurance solutions. Backed by Mubadala/RedBird. $4.1B Brighthouse acquisition (2025).',
  },
];
...
async function checkIfExists(sheets, companyName) {
  ...
  const normalized = companyName.toLowerCase().replace(/ llc| lp| partners?| capital| group/gi, '').trim();
  
  for (const company of companies) {
    const normalizedExisting = company.toLowerCase().replace(/ llc| lp| partners?| capital| group/gi, '').trim();
...
async function apolloSearch(domain, companyName) {
  ...
  const body = {
    ...
    person_titles: [
      ...
      'Co-Founder', 'Founder', 'President', 'Partner',
...
async function addNewFirms() {
  ...
  for (const firm of newFirms) {
    ...
    if (exists) {
      console.log(`  ⏭️  Already in sheet (or similar name), skipping`);
      continue;
    }
  ...
  if (rowsToAdd.length > 0) {
    ...
    console.log(`✅ New firms added to CRM sheet!`);
  } else {
