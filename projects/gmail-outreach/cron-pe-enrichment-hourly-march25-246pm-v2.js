...
async function searchApolloContacts(company, website) {
  try {
    ...
    // Search by domain - simpler, more reliable
...
async function main() {
  ...
  for (const target of targets.slice(0, 12)) {
    ...
    console.log('Searching Apollo API (by domain)...');
    ...
    if (contacts.length > 0) {
      ...
      const notes = `Apollo API (2026-03-25) - ${contacts.length} contacts found. Source: ${best.verified ? 'Verified' : 'Unverified'} email.`;
