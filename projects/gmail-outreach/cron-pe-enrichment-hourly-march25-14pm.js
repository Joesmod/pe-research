...
async function main() {
  console.log('🫡 PE Research & Enrichment - Hourly Run (Wed March 25, 2026 - 2:16 PM CST)\n');
  ...
  for (const target of targets.slice(0, 12)) {
    ...
    if (contacts.length > 0) {
      ...
      const notes = `Apollo API (${new Date().toISOString().split('T')[0]}) - ${contacts.length} contacts found. Best: ${best.verified ? 'Verified' : 'Unverified'} email.`;
      ...
      await sheets.spreadsheets.values.update({
        ...
        valueInputOption: 'RAW',
        requestBody: {
          values: [[`Apollo search: No contacts found (${new Date().toISOString().split('T')[0]}). Try manual research.`]]
        }
      });
    }
  }
