...
async function main() {
  console.log('🫡 PE Enrichment - Priority Firms\n');
  
  // Load targets
  const targets = JSON.parse(fs.readFileSync('_enrichment_targets.json', 'utf-8'));
  
  // Filter for high-priority firms - "Needs Enrichment" or "New" status
  const priority = targets.filter(t => 
    t.status === 'Needs Enrichment' || 
    t.status === 'New' ||
    (t.status && t.status.startsWith('New'))
  );
  
  console.log(`Total targets: ${targets.length}`);
  console.log(`Priority firms (Needs Enrichment / New): ${priority.length}`);
  ...
  for (let idx = 0; idx < Math.min(15, priority.length); idx++) {
    const target = priority[idx];
    const { rowIndex, company, website } = target;
    
    console.log(`[${idx+1}/15] ${company}`);
    console.log(`  Website: ${website || 'N/A'}`);
    console.log(`  Current status: ${target.status || 'New'}`);
  ...
  console.log('📊 PRIORITY ENRICHMENT SUMMARY');
  console.log('='.repeat(60));
