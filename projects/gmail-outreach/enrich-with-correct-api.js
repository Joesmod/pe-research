...
// Use the CORRECT endpoint: /api/v1/mixed_people/api_search
async function searchApolloPeople(domain, companyName) {
  if (!domain) return [];
  
  try {
    const response = await axios.post(
      'https://api.apollo.io/api/v1/mixed_people/api_search',  // CORRECTED ENDPOINT
      {
...
async function enrichLeads() {
  ...
  for (const lead of toEnrich) {
    ...
    if (!domain) {
      console.log(`   🔍 Searching Apollo for org...`);
    ...
    if (!domain) {
      ...
      updates.push({
        ...
        values: [['Apollo search attempted - no domain/contact found - manual research needed - 2026-03-09']]
      });
      continue;
    }
