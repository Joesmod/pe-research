...
// NEW Apollo API endpoint (api_search instead of search)
...
async function main() {
  console.log('=== PE Research & Enrichment Cron - March 7, 4:06 AM (v3 - New API) ===\n');
  
  const rows = await readSheet();
  console.log(`Total rows: ${rows.length}`);
  
  const header = rows[0];
  console.log('Headers:', header.slice(0, 7).join(' | '));
  ...
  for (const lead of batch) {
    ...
    if (lead.website && lead.website.startsWith('http')) {
      try {
        ...
        console.log(`   🔎 Apollo API search: ${domain}`);
        ...
        if (people.length > 0) {
          ...
          if (best && best.email) {
            ...
            await updateRow(lead.rowIndex, {
              ...
              status: 'Enriched',
              notes: `Apollo API enrichment ${new Date().toISOString().split('T')[0]}`
            });
          ...
          console.log(`   ⚠️ No contacts found in Apollo for ${domain}`);
          failed++;
        }
      ...
      console.log(`   ⚠️ Invalid or missing website`);
      failed++;
    }
  }
  ...
  if (needsEnrichment.length > batch.length) {
    console.log('\n💡 Note: More leads need enrichment. Run again or manually research.');
  }
}
