...
function needsEnrichment(row, index) {
  ...
  // Only process rows that are NOT Dead, Bounced, or Enriched
  const isDead = status.toLowerCase().includes('dead') || 
                 status.toLowerCase().includes('bounced') ||
  ...
  const hasEmptyOrGenericEmail = !email || email.includes('info@') || 
  ...
  return hasEmptyContact || hasEmptyOrGenericEmail;
}
...
async function main() {
  ...
  for (let i = 1; i < rows.length; i++) {
    if (needsEnrichment(rows[i], i)) {
      targets.push({
        ...
        website: rows[i][5] || '',
        linkedin: rows[i][6] || '',
        status: rows[i][9] || 'Empty',
      });
    }
  }
  
  console.log(`Found ${targets.length} active leads needing enrichment\n`);
  ...
  console.log(`\nBeginning web research for ${enrichmentTargets.length} firms...`);
