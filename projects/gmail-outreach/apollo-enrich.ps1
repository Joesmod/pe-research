...
Write-Host "Enriching $($highPriority.Count) high-priority PE firms via Apollo API...`n"
...
foreach ($firm in $highPriority) {
    ...
    try {
        ...
        if ($response.people -and $response.people.Count -gt 0) {
            $person = $response.people[0]  # Take first result (usually highest ranking)
            ...
            Write-Host "   ✗ No contacts found`n"
        }
        
        Start-Sleep -Milliseconds 500  # Rate limiting
        
    } catch {
        Write-Host "   ✗ API Error: $($_.Exception.Message)`n"
    }
}

Write-Host "`nEnriched $($enriched.Count) firms successfully"
