/**
 * Hourly PE Enrichment Cron - March 17, 2026 11:37 PM
...
async function apolloSearch(companyDomain, companyName) {
  ...
  const body = {
    ...
    person_titles: [
      ...
      'Director of Product', 'Director of Marketing', 'Director of Business Development',
      'VP Technology', 'VP Digital', 'VP Operations', 'VP Digital Transformation',
...
async function enrichLeads() {
    ...
    try {
      ...
      if (result) {
        ...
        if (colNotes !== null) {
          ...
          updatedRow[colNotes] = `${existingNotes ? existingNotes + '; ' : ''}Enriched via ${result.source} on 2026-03-17`;
        }
        ...
        enrichmentLog.push({
          company: lead.company,
          status: 'No Apollo results - needs manual research',
        });
      }
  ...
  // Write log
  const fs = require('fs');
  const logPath = path.join(__dirname, 'CRON-ENRICHMENT-REPORT-MARCH17-11PM.md');
  const logContent = `# PE Enrichment Report - March 17, 2026 11:37 PM

## Summary
- **Total rows scanned:** ${rows.length - 1}
- **Firms needing enrichment:** ${needsEnrichment.length}
- **Firms attempted:** ${toEnrich.length}
- **Successfully enriched:** ${successCount}
- **Manual research needed:** ${toEnrich.length - successCount}

## Enriched Contacts

${enrichmentLog.filter(e => e.contact).map(e => `### ${e.company}
- **Contact:** ${e.contact}
- **Title:** ${e.title}
- **Email:** ${e.email}
...
${enrichmentLog.filter(e => e.status || e.error).map(e => `- **${e.company}**: ${e.status || e.error}`).join('\n')}
...
`;
  ...
  return { successCount, totalAttempted: toEnrich.length, enrichmentLog };
}
