...
async function searchApolloPeople(orgId, companyName) {
  try {
    const response = await axios.post(
...
async function enrichLeads() {
  ...
  for (const lead of toEnrich) {
    ...
    // First, find the org
    const org = await searchApolloOrg(lead.company, lead.website);
    ...
    const people = await searchApolloPeople(org.id, lead.company);
    
    if (people.length > 0) {
      const bestContact = people[0];
      console.log(`   ✅ ${bestContact.name} - ${bestContact.title}`);
      console.log(`   📧 ${bestContact.email}`);
      console.log(`   🔗 ${bestContact.linkedin_url || 'No LinkedIn'}`);
