...
async function searchApolloContacts(company, website) {
  try {
    ...
    // Use the correct api_search endpoint with header auth
...
async function main() {
  ...
  for (const target of targets.slice(0, 12)) {
    ...
    if (contacts.length > 0) {
      ...
      // Pick best: verified email preferred, then partner/principal, then highest title
      ...
      const notes = `Apollo API (2026-03-25 2:46pm) - ${contacts.length} contacts found. Source: ${best.verified ? 'Verified' : 'Unverified'} email.`;
      ...
      await sheets.spreadsheets.values.update({
        ...
        valueInputOption: 'RAW',
        requestBody: {
          values: [[`Apollo search: No contacts found (2026-03-25). Firm not in Apollo database. Manual research needed: website team pages, LinkedIn site: search, or press releases.`]]
        }
      });
