...
const leadsToEnrich = JSON.parse(fs.readFileSync('leads-to-enrich-706pm-march6.json', 'utf8'));
...
async function enrichLeads() {
  ...
  // Save results
  fs.writeFileSync('apollo-enrichment-706pm-march6.json', JSON.stringify(results, null, 2));
  console.log(`\n\n=== ENRICHMENT COMPLETE ===`);
  console.log(`Total processed: ${results.length}`);
  console.log(`Successfully enriched: ${results.filter(r => r.email).length}`);
  console.log(`Results saved to: apollo-enrichment-706pm-march6.json`);
