...
// More obscure mid-market PE firms, services-focused
const TARGET_FIRMS = [
  'Gryphon Investors',
...
async function main() {
  ...
  console.log(`${existing.size} existing firms`);
  ...
  for (const firmName of TARGET_FIRMS) {
    ...
    const org = orgs[0];
    // Verify it's actually the right firm
    if (!org.name.toLowerCase().includes(firmName.split(' ')[0].toLowerCase())) { console.log(`Mismatch: ${org.name}`); continue; }
