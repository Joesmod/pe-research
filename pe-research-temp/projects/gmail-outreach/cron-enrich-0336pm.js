...
const targets = JSON.parse(fs.readFileSync('selected-targets-0336pm.json', 'utf8'));
...
async function main() {
  console.log('=== PE ENRICHMENT CRON RUN (3:36 PM) ===');
  ...
  for (const target of targets) {
    ...
    if (people.length > 0) {
      ...
      if (email && !email.match(/^(info|sales|ir|contact|admin|support)@/i)) {
        updates.push({
  ...
  // Save log
  const logFile = 'enrichment-log-0336pm.json';
  ...
  console.log(`With verified emails: ${updates.filter(u => u.email).length}`);
  console.log(`Partial updates: ${updates.filter(u => u.partial).length}`);
