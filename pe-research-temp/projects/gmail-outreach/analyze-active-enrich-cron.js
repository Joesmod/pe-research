...
async function analyzeSheet() {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    // Skip if already contacted, dead lead, duplicate, or "Researched - No Email"
    if (!companyName || 
        status === 'Contacted' || 
        status === 'Dead Lead' || 
        status === 'DUPLICATE' ||
        status === 'Researched - No Email') continue;
    
    // Check if needs enrichment
    const hasNoContact = !contactName || contactName.trim() === '' || contactName === 'Not identified';
  ...
  console.log(`Total active firms needing enrichment: ${needsEnrichment.length}\n`);
  console.log('First 15 active firms to enrich:\n');
